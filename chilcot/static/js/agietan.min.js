...
font_size=14
while(($("#mobile-navbar-header").height()>($("#mobile-title").height()*3))){font_size=font_size-1
...
$("#"+parent_id+".dropdown-toggle").dropdown("toggle");});$('.nav-link').on('click touchend',function(){$('.navbar-collapse').removeClass('in');});function getParameterByName(name,url){if(!url)url=window.location.href;name=name.replace(/[\[\]]/g,"\\$&");var regex=new RegExp("[?&]"+name+"(=([^&#]*)|&|#|$)"),results=regex.exec(url);if(!results)return null;if(!results[2])return'';return decodeURIComponent(results[2].replace(/\+/g," "));}
