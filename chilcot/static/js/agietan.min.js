hide_state=true;if(hide_state==true){$(".extended").hide();$(".extended-row").hide();$('#master_expand_link').text("Show All Notes");}else{$('#master_expand_link').text("Hide All Notes");$(".expand-link").hide();};if(!String.prototype.startsWith){String.prototype.startsWith=function(searchString,position){position=position||0;return this.indexOf(searchString,position)===position;};}
...
font_size=14
while(($("#mobile-navbar-header").height()>($("#mobile-title").height()*3))){font_size=font_size-2
...
adjustMenus(id);}
$("a.top-nav-item").mouseup(function(){id=$(this).attr('section');title=$("#"+id+".section-anchor").text()
...
if(direction=="down"){id=$(this.element).attr('id');title=$(this.element).text()}else{id=$(this.element).attr('prev');title=$("#"+id+".section-anchor").text()};changeSelected(id,title);},{offset:function(){if($(this.element).is(':visible')==true){return $(this.element).height()+60;}else{return 0}},continuous:false});$('.expand-footnotes').on('click touchend',function(event){event.preventDefault();id=$(this).attr('id');$("#"+id+".footnotes").animate({height:'toggle',opacity:'toggle'},'slow');window.setTimeout(Waypoint.refreshAll,500);});$('.hide-footnotes').on('click touchend',function(event){event.preventDefault();id=$(this).parent().parent().attr('id');$("#"+id+".footnotes").animate({height:'toggle',opacity:'toggle'},'slow');window.setTimeout(Waypoint.refreshAll,500);});$('.expand-link').on('click touchend',function(event){event.preventDefault();$(this).fadeOut();id=$(this).attr('id');$("#"+id+".extended-row").show();$("#"+id+".extended").animate({height:'toggle',opacity:'toggle'},'slow');window.setTimeout(Waypoint.refreshAll,500);});$('.hide-link').on('click touchend',function(event){event.preventDefault();id=$(this).attr('id');$("#"+id+".extended-row").hide();$("#"+id+".extended").animate({height:'toggle',opacity:'toggle'},'slow');$("#"+id+".expand-link").fadeIn();window.setTimeout(Waypoint.refreshAll,500);});$(".content-row").on('mouseenter',function(){id=$(this).attr('id')
$(".cite-link").not(".manual_paragraph").hide();$('#'+id+".cite-link").show();})
var last_id=-1
...
existing.attr('temp_name',newhash);existing.attr('name',"temp");window.location.hash=newhash;window.setTimeout(function(){existing.attr('name',newhash)},1);}}}}
...
function gotoHash(hash,addText){key_areas=hash.split(".")
if(key_areas[0]=="#"){key_areas[0]=key_areas[0].substring(1)}
internalMove=false
...
$('a').each(function(){var a=new RegExp('/'+window.location.host+'/');if(!a.test(this.href)){$(this).click(function(event){event.preventDefault();event.stopPropagation();window.open(this.href,'_blank');});}});$(function(){$('a[href*=#]:not([href=#])').on('click touchend',function(){if(location.pathname.replace(/^\//,'')==this.pathname.replace(/^\//,'')&&location.hostname==this.hostname&&location.pathname==this.pathname&&$(this).hasClass('cite-copy')==false){var splits=this.hash.split(".")
...
if(new_hash=="index"){if($("div#index").is(':visible')==false){$("div#index").show()
...
function citeBox(dialog_title,link,para_tag){title=$("body").attr("title")
...
'<br><br>Cite As:<br><span class="cite"> '+cite_author+' ('+year+'), <i>'+full_title+'</i>. [online] , '+org+'. '+ele_name+': '+ele_id+', Available at: '+link+' [Accessed '+getNow()+']</span>',html:true,confirmButtonText:"OK"});}
$(".cite-copy").on('click touchend',function(e){link=$(this).attr("href")
port=window.location.port;if(port!=""){port=":"+port}
if(link.indexOf("#")!==-1){link=window.location.protocol+"//"+window.location.hostname+port+window.location.pathname+link}else{link=window.location.protocol+"//"+window.location.hostname+port+link}
...
$("#"+parent_id+".dropdown-toggle").dropdown("toggle");window.location.href=this.href;return false});$('.nav-link').on('click',function(){window.location.href=this.href;$('.navbar-collapse').removeClass('in');return false});function getParameterByName(name,url){if(!url)url=window.location.href;name=name.replace(/[\[\]]/g,"\\$&");var regex=new RegExp("[?&]"+name+"(=([^&#]*)|&|#|$)"),results=regex.exec(url);if(!results)return null;if(!results[2])return'';return decodeURIComponent(results[2].replace(/\+/g," "));}
$(window).load(function(){if(window.location.search){hash_loc=getParameterByName("para")
